<script>
	...
	onMount(() => {
		window.addEventListener('click', (e) => {
			let target = e.target.closest('.work-mode__time')
			// console.log(e.target.closest('.work-mode__time'))
		})
	})

	function inputHour(e) {
	...
	function changeHour(e) {
		...
		if(+el.value < 10) el.value = '0'+el.value
	}

	function inputMinute(e) {
		let el = e.currentTarget
		el.value = el.value.replace(/\D/g, '')
		if(el.value.length > 2) el.value = el.value.slice(0,2)
	}
	...
	function selectHour(e) {
		...
		let inputHour = container.querySelector('.work-mode__time-hour')
		inputHour.value = el.textContent
	}

	function selectMinute(e) {
		let el = e.currentTarget
		let container = el.closest('.work-mode__time')
		let inputHour = container.querySelector('.work-mode__time-minute')
		inputHour.value = el.textContent
	}
</script>
			...
			<div class='work-mode__item'>
				<div class='work-mode__item-header'>
					<label class='work-mode__item-label'>
						...
						<input type='checkbox' class='work-mode__item-checkbox' bind:group={checked} value={day.value}>
					</label>
				...
				<div class='work-mode__item-content' class:work-mode__disabled={!checked.find(el => el == day.value)}>
					...
					<div class='work-mode__times'>
						...
						<div class='work-mode__times-inner'>
							<button type="button" class='work-mode__time' on:click={showList}>
								<input type='number' class="work-mode__time-input work-mode__time-hour" value="00" on:input={inputHour} on:change={changeHour}>
								<span class="work-mode__time-delimiter">:</span>
								<input type='number' class="work-mode__time-input work-mode__time-minute" value="00" on:input={inputMinute} on:change={changeMinute}>
								<div class='work-mode__time-list' style:display={list1 ? 'flex': 'none'}>
									<div class='work-mode__time-list-hours work-mode__time-list-items'>
										{#each dataHours as hour}
											<button type="button" class='work-mode__time-list-hour work-mode__time-list-item' on:click={selectHour}>{hour}</button>
										{/each}
									</div>
									<div class='work-mode__time-list-minutes work-mode__time-list-items'>
										{#each dataMinutes as minute}
											<button type="button" class='work-mode__time-list-hour work-mode__time-list-item' on:click={selectMinute}>{minute}</button>
										{/each}
									</div>
								</div>
							</button>
							<span class='work-mode__times-delimiter'>-</span>
							<div class='work-mode__time'>
								<input type='number' class="work-mode__time-input work-mode__time-hour" value="00">
								<span class="work-mode__time-delimiter">:</span>
								<input type='number' class="work-mode__time-input work-mode__time-minut" value="00">
							</div>
						</div>
					</div>
					<div class='work-mode__range-buttons'>
						...
						<div class='work-mode__range-buttons-inner'>
							<button type="button" class='work-mode__range-button'>All days</button>
							<button type="button" class='work-mode__range-button'>Weekdays</button>
						</div>
					</div>
					<div class='work-mode__controls-buttons'>
						<label for={day.value} type="button" class='work-mode__controls-button'>Cancel</label>
						<label for={day.value} type="button" class='work-mode__controls-button'>Save</label>
					</div>
				</div>
			</div>
        {/each}
